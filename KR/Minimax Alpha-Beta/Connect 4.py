...
class Joc:
    ...
    def final(self):
        ...
        for i in range(Joc.NR_LINII):
            for j in range(Joc.NR_COLOANE - Joc.NR_CONNECT + 1):
                linie = set(self.matr[i * Joc.NR_COLOANE + j: i * Joc.NR_COLOANE + j + Joc.NR_CONNECT])
                if len(linie) == 1 and list(linie)[0] != Joc.GOL:
                    castigator = list(linie)[0]
                    return castigator
        pass
        ...
        for i in range(Joc.NR_LINII - Joc.NR_CONNECT + 1):
            for j in range(Joc.NR_COLOANE):
                coloana = []
                for connect in range(Joc.NR_CONNECT):
                    coloana.append(self.matr[(i + connect) * Joc.NR_COLOANE + j])
                coloana = set(coloana)
                if len(coloana) == 1 and list(coloana)[0] != Joc.GOL:
                    castigator = list(coloana)[0]
                    return castigator
        pass
        ...
        for i in range(Joc.NR_LINII - Joc.NR_CONNECT + 1):
            for j in range(Joc.NR_COLOANE - Joc.NR_CONNECT + 1):
                diagonala = [self.matr[i * Joc.NR_COLOANE + j]]
                for connect in range(Joc.NR_CONNECT - 1):
                    diagonala.append(self.matr[(i + connect + 1) * Joc.NR_COLOANE + j + connect + 1])
                diagonala = set(diagonala)
                if len(diagonala) == 1 and list(diagonala)[0] != Joc.GOL:
                    castigator = list(diagonala)[0]
                    return castigator
        pass

        # verificam diagonale /
        for i in range(Joc.NR_LINII - Joc.NR_CONNECT + 1):
            for j in range(Joc.NR_CONNECT - 1, Joc.NR_COLOANE):
                diagonala = [self.matr[i * Joc.NR_COLOANE + j]]
                for connect in range(Joc.NR_CONNECT - 1):
                    diagonala.append(self.matr[(i + connect + 1) * Joc.NR_COLOANE + j - connect - 1])
                diagonala = set(diagonala)
    ...
    def mutari_joc(self, jucator_opus):
        ...
        for coloana in range(Joc.NR_COLOANE):
            linie = 0
            while self.matr[linie * Joc.NR_COLOANE + coloana] == Joc.GOL and linie < Joc.NR_LINII - 1:
                linie += 1
            if linie != 0:
                matr_tabla_noua = copy.deepcopy(self.matr)
                if linie == Joc.NR_LINII - 1 and matr_tabla_noua[linie * Joc.NR_COLOANE + coloana] == Joc.GOL:
                    matr_tabla_noua[linie * Joc.NR_COLOANE + coloana] = jucator_opus
                else:
                    matr_tabla_noua[(linie - 1) * Joc.NR_COLOANE + coloana] = jucator_opus
            else:
                continue
            joc = Joc(matr_tabla_noua)
            l_mutari.append(joc)
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        for i in range(Joc.NR_LINII):
            for j in range(Joc.NR_COLOANE - Joc.NR_CONNECT + 1):
                linie = self.matr[i * Joc.NR_COLOANE + j: i * Joc.NR_COLOANE + j + Joc.NR_CONNECT - 1]
                if juc_opus in linie:
                    continue
                rez += 1
        ...
        for i in range(Joc.NR_LINII - Joc.NR_CONNECT):
            for j in range(Joc.NR_COLOANE - Joc.NR_CONNECT):
                diagonala = []
                for connect in range(Joc.NR_CONNECT):
                    diagonala.append(self.matr[(i + connect) * Joc.NR_COLOANE + (j + connect)])
                if juc_opus in diagonala:
                    continue
                rez += 1
            ...
            while not raspuns_valid:
                try:
                    ...
                    if coloana < 0 or coloana > Joc.NR_COLOANE:
                        ...
                        for i in range(Joc.NR_COLOANE - 1):
